...
- The local backend listens on `127.0.0.1:8765`; the default `PUBLIC_BASE_URL` is now aligned to `http://127.0.0.1:8765` so generated OAuth callback URLs match the local development server unless `.env` explicitly overrides it.
