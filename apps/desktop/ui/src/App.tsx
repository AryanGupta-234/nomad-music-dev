...
// NOTE: these were previously declared *inside* the App() component as closures,
// but are called from module-level components (MusicCard, TrackRow, Player,
// QueueDrawer, HomePage) that render outside App's scope. That produced an
// "artStyle is not defined" / "artistOf is not defined" ReferenceError at
// render time, which crashed almost every screen (home, library, search,
// player, queue) before it ever got to paint artwork, icons, or handle
// playback. Promoting them to module scope fixes the crash.
...
function SpotifyIcon({ size = 14 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true">
    ...
    <path d="M6.4 9.4c3.6-1 8-.7 10.9 1a.85.85 0 1 1-.9 1.45c-2.5-1.5-6.4-1.8-9.6-.9a.85.85 0 1 1-.4-1.65Zm-.3 3.15c3-.85 7.1-.55 9.7.95a.75.75 0 1 1-.76 1.3c-2.3-1.3-6-1.55-8.6-.83a.75.75 0 1 1-.34-1.42Zm-.3 3.05c2.5-.7 5.9-.4 8.1.9a.65.65 0 0 1-.68 1.1c-1.95-1.15-4.95-1.4-7.15-.8a.65.65 0 0 1-.27-1.2Z" fill="#06100d" />
  </svg>;
}
function YouTubeIcon({ size = 14 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true">
    <rect x="0" y="3" width="24" height="18" rx="6" fill="#FF0033" />
    <path d="M10 8.3 16.5 12 10 15.7Z" fill="#fff" />
  </svg>;
}
function LocalIcon({ size = 14 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true">
    <circle cx="12" cy="12" r="12" fill="#3d434b" />
    <path d="M8 15V9l8-1.6V13" stroke="#e9edee" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" fill="none" />
    <circle cx="8" cy="16" r="1.9" fill="#e9edee" />
    <circle cx="16" cy="14.4" r="1.9" fill="#e9edee" />
  </svg>;
}
function ProviderIcon({ provider, size = 14 }: { provider: string; size?: number }) {
  if (provider === "spotify") return <SpotifyIcon size={size} />;
  if (provider === "youtube") return <YouTubeIcon size={size} />;
  if (provider === "local") return <LocalIcon size={size} />;
  return <span className="provider-dot" style={{ width: size * 0.6, height: size * 0.6 }} />;
}
function ProviderBadge({ provider }: { provider: string }) {
  return <span className="provider-badge" title={provider}><ProviderIcon provider={provider} size={11} />{provider}</span>;
}
...
export default function App() {
  ...
  async function resolveAndQueue(track: Track, append = false) {
    ...
    try {
      ...
      } else if (d.reason === "provider_not_connected") {
        const missing = Array.from(new Set((d.candidates || []).map((c: { provider: string }) => c.provider))) as string[];
        flash(`Connect ${missing.map(p=>`${p[0].toUpperCase()}${p.slice(1)}`).join(" / ")} in Source Hub to play this track.`);
      } else if (d.reason === "no_playback_source") {
        flash("No playable source found for this track yet — try syncing providers.");
  ...
  return <div className="nomad-app">
    <aside className="sidebar">
      ...
      <button className="connection-card" onClick={() => setShowConnections(true)}><div className="connection-title"><span>SOURCE HUB</span><span>→</span></div>{["spotify","youtube"].map(name => { const s = providerState(name); return <div className="connection-mini-row" key={name}><i className={`status-led ${s.tone}`}/><b><ProviderIcon provider={name} size={13}/>{name}</b><small>{s.label}</small></div> })}</button>
